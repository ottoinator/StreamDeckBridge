...
Die beiden US Paper-Lane-Views lesen zuerst die redigierte Live-Broker-Projektion aus `/api/v1/view/streamdeck-tiles`. Damit bleiben Fill, Broker-Ablehnung und ausstehende Provision intraday sichtbar; PnL erscheint erst bei gebuchter Abrechnung. Fehlt der neue Vertrag waehrend eines gestaffelten Deployments, faellt die Bridge read-only auf die Promotion-Evidenz aus `/api/v1/view/observer-card` zurueck. Mamba Transfer kommt weiterhin aus dem Observer-Endpoint und muss `pnl_kind: "what_if"` liefern. Native95 und ORB13 werden nie addiert; fehlende oder pending Abrechnung bleibt `n/a`, niemals `0,00 EUR`. Die Bridge liest weder Service-Units noch Runtime-Umgebungen und beschafft keine Tokens per SSH.

Ein frischer Broker-Paper-Snapshot mit explizitem `accounting.status=no_trades`, leerer Trade-Liste und null Fill/Pending/Reject ist ein valider terminaler Strategieausgang. Die ORB13-Kacheln zeigen dann `NO TRADES` mit Status `ok`; nur eine leere Liste ohne diesen Producer-Status sowie fehlende, ungueltige oder veraltete Evidenz bleibt fail-closed `UNAVAILABLE`/`warn`. Ein Null-PnL wird dabei nicht erfunden: Die PnL-Kachel bleibt `n/a`, bis gebuchte Broker-Abrechnung vorliegt.

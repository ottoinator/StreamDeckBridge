...
Ein Druck auf `Noah Live Markets` schaltet alle fuenf Noah-Kacheln read-only durch diese Ansichten: `Native95 Fixed60 Primary -> ORB13 Paper Challenger -> Mamba Transfer 52→95 What-if -> MLB Elo V2 -> MLB Teamform -> Native95 Fixed60 Primary`. BTC und Weather sind nicht mehr auswaehlbar. Die Auswahl wird lokal in `noah-view.json` gespeichert; sie aktiviert keine Runtime, keine Scheduler und keine Trading-/Order-Pfade. Die alten Auswahl-Keys `us` und `mamba_native95` werden beim Lesen sicher auf `paper_primary` migriert; alte BTC-, Weather-, Crypto-, Prediction- oder Combined-Ansichten ebenfalls.
...
MLB Elo V2 liest ausschliesslich die aktiven lokalen paper-only Continuity-, Capture- und Ledger-Projektionen aus `mlb-elo-v2-confirmatory-v2`. MLB Teamform liest den `status.json`-Vertrag des laufenden `wm-vorhersager-mlb-nextgen-team-form-paper`-Containers und prueft Paper-/Shadow-Authority, Ledger, Cache, Settlement und Frische fail-closed. Die Paper-PnL-Kachel zeigt fuer Teamform bewusst nur die kumulierte Paper-PnL (`PAPER TOTAL`); Tages-PnL und Paper-Trades bleiben `n/a`, solange der Teamform-Vertrag keine autoritativen Tagesfenster liefert. Die Settlement-Anzahl erscheint nur neben `PAPER TOTAL`, nie als heutiger Trade-Count. Fehlende, veraltete oder authority-unsichere Projektionen werden sichtbar blockiert; es gibt keinen Rueckfall auf andere Maerkte oder aggregierte PnL.

Der Teamform-Root kann reproduzierbar mit `CODEX_MONITOR_MLB_TEAM_FORM_V3_ROOT` gesetzt werden. Ohne Override ermittelt die Bridge den aktiven `/lane`-Mount read-only per `docker inspect` am Container `wm-vorhersager-mlb-nextgen-team-form-paper`; ein alternativer Containername kann ueber `CODEX_MONITOR_MLB_TEAM_FORM_V3_CONTAINER` gesetzt werden.
